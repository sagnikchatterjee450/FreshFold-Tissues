...
const Orders: React.FC<OrdersProps> = ({ state, updateState }) => {
  ...
  const generatePDF = async (order: Order) => {
    ...
    // Add Payment QR Code (Above signature area in blank space)
    try {
      const qrCodeSize = 35;
      const qrCodeX = pageWidth - margin - qrCodeSize;
      const qrCodeY = footerY - 80; // Position higher above signature lines
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (typeof dataUrl === 'string') {
            ...
            doc.setFont("helvetica", "bold");
            doc.setTextColor(79, 70, 229);
            doc.text("Scan for Payment", qrCodeX + (qrCodeSize / 2), qrCodeY + qrCodeSize + 3, { align: 'center' });
          }
        }
      } catch (e) {
